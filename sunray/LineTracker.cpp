...
float stanleyTrackingSlowP = 0.5;    
...
{
  bool isClose = fabs(maps.distanceToPerimeter(position)) < 0.1;
  return isClose && maps.wayMode != WAY_DOCK;   
}
...
bool shouldUseRearTraction(vec3_t target)
{
  if (maps.isDocking())         // forward traction if docking
    return false;
  else if (maps.isUndocking())  // rear traction if undocking
    return true;
  //else if (distance(target, vec3_t(-1.86, -2.01)) < 0.1)  // exceptions
    //return true;

  return maps.distanceToPerimeter(target) > 0.1;
}
...
bool isStuck()
{
  ...
  // check every 2 seconds for lack of movement
  if (now - lastStuckTime > 2000)
  {
  ...
  // if no movement for 5 tries return true
  if (stuckCounter >= 5)
  {
    stuckCounter = 0;
    return true;
...
void trackLine(bool runControl)
{
  ...
  // rear traction
  maps.trackReverse = shouldUseRearTraction(target);
  ...
  float ra = 45.0;
  if (maps.isDocking()) ra = 10.0;
  if (!stillRotation && fabs(trackerDiffDelta) > radians(ra))
  {
    //stillRotation = true;
  ...
  if (stillRotation)
  {
    ...
    float stoppingDistance = 0.5 * sq(motor.angularSpeedSet) / ANGULAR_ACCELERATION;
    ...
    // stop
    if (decelerateAngular && fabs(motor.angularSpeedSet) < 0.06)
    {
      stillRotation     = false;
  ...
  else
  {
    ...
    linear *= expf(-trackerDiffDelta * trackerDiffDelta * HEADING_ERROR_SPEED_FACTOR); // slow down on heading error
    //linear *= 1.0 - fabs(trackerDiffDelta / PI + 0.1) / 1.1;
    if (maps.trackReverse)
      linear *= -1.0;   // reverse line tracking needs negative speed
    else if (!maps.isDocking())
      linear *= 1.0 - constrain(-imuDriver.pitch * sign(motor.linearSpeedSet) / (PI*0.5) * 4.0 - 0.1, 0.0, 1.0); // slow down on uphill
    
    // linear deceleration
    float stoppingDistance = 0.5 * sq(motor.linearSpeedSet) / LINEAR_ACCELERATION;
    
    if (!decelerateLinear && targetDist <= max(stoppingDistance, 0.01) || dirDot <= 0.0)
  ...
  // kidnap detect
  if (KIDNAP_DETECT
  && fabs(distToPath) > KIDNAP_DETECT_ALLOWED_PATH_TOLERANCE
  && !maps.isInsidePerimeter(position.x, position.y)
  && !maps.isUndocking())
  ...
  // target reached
  if ((decelerateLinear && fabs(motor.linearSpeedSet) < 0.02) || dirDot <= 0.0 || distance(lastTarget, target) < 0.001)
  {
