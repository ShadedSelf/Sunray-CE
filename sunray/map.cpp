...
void NodeList::dealloc(){
  ...
  //if (nodes[numNodes].point != CHECK_POINT)
    //memoryCorruptions++;
  delete[] nodes;
  nodes = NULL;
  numNodes = 0;  
    ...
    case WAY_PERIMETER:            
      if (perimeterPoints.alloc(count)){
        for (int i=0; i < count; i++){
          int sidx = i;
          if (sidx < points.numPoints)
            perimeterPoints.points[i].assign( points.points[sidx] );
        }
      }
      break;
    case WAY_EXCLUSION:      
    ...
    case WAY_DOCK:    
      if (dockPoints.alloc(count)){
        for (int i=0; i < count; i++){
          ...
          if (sidx < points.numPoints) 
            dockPoints.points[i].assign( points.points[ sidx ] );
        }
      }
      break;
    case WAY_MOW:          
      if (mowPoints.alloc(count)){
        for (int i=0; i < count; i++){
          ...
          if (sidx < points.numPoints)
            mowPoints.points[i].assign( points.points[ sidx ] );
        }
...
bool Map::setExclusionLength(int idx, int len){  
  ...
  for (int j=0; j < len; j++){
    ...
    if (sidx < points.numPoints)
      exclusions.polygons[idx].points[j].assign( points.points[ sidx ] );
    ptIdx ++;
  }
  CONSOLE.print("ptIdx=");
  CONSOLE.print(ptIdx);
...
int Map::findNextNeighbor(NodeList &nodes, PolygonList &obstacles, Node &node, int startIdx)
{
  for (int idx = startIdx+1; idx < nodes.numNodes; idx++)
  {
    ...
    if (&nodes.nodes[idx] == &node) continue;
    ...
    {                                
      ...
      if (isPeri)
      { 
        if (!pointIsInsidePolygon(obstacles.polygons[idx3], node.point)) // start point outside perimeter?  
        {                                                                                     
          if (linePolygonIntersectPoint(node.point, *pt, obstacles.polygons[idx3], sectPt))
          {                       
            if (distance(node.point, sectPt) > ALLOW_ROUTE_OUTSIDE_PERI_METER) { 
              safe = false; break; } // entering perimeter with long distance is not safe
            continue;           
          }
...
bool Map::findPath(Point src, Point dst){
  ...
  if (ENABLE_PATH_FINDER){    
    ...
    for (int i=0; i < exclusions.numPolygons; i++){
      if (!polygonOffset(exclusions.polygons[i], pathFinderObstacles.polygons[idx], -0.04))
        return false;
      idx++;
    }      
    for (int i=0; i < obstacles.numPolygons; i++){
      if (!polygonOffset(obstacles.polygons[i], pathFinderObstacles.polygons[idx], -0.04))
        return false;
      idx++;
    }  
    ...
    {       
      if (millis() >= nextProgressTime){
        nextProgressTime = millis() + WATCHDOG_TIMER/5;          
        CONSOLE.print(".");
        watchdogReset();     
      }
      ...
      if (distance(currentNode->point, end->point) < 0.02)
        break;        
