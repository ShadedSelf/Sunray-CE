...
SFE_UBLOX_GNSS_SERIAL configGPS; // used for f9p module configuration only
  ...
  for (int idx=0; idx < 3; idx++){
    for (int i=1; i < 3; i++){
      ...
      if (idx == 0){
        ...
        setValueSuccess &= configGPS.addCfgValset8(UBLOX_CFG_MSGOUT_NMEA_ID_GSV_UART1, 0);  // GSV
        setValueSuccess &= configGPS.addCfgValset8(UBLOX_CFG_MSGOUT_NMEA_ID_GSV_UART2, 0);  // GSV
